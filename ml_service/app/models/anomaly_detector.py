"""
Model 2: Trip Anomaly Detection
- Primary: Isolation Forest (unsupervised)
- Secondary: Local Outlier Factor for comparison
- Detects: unusual trip durations, route deviations, speed anomalies
- Output: anomaly_score (0-1), is_anomalous, contributing_factors
"""
...
ANOMALY_FEATURE_COLS = [
    ...
    "hour_deviation",        # how far from peak hour for this route
    "is_night_trip",
]


def fetch_anomaly_data(conn) -> pd.DataFrame:
...
def train(conn, models_dir: Path) -> dict:
    ...
    # For each anomalous trip, which features are most extreme?
...
def predict(artifact: dict, trip_data: dict) -> dict:
    ...
    features = pd.DataFrame([{
        ...
        "eta_delay_minutes": trip_data.get("eta_delay_minutes", 0),
        "duration_ratio": trip_data.get("duration_ratio", 1.0),
        "delay_ratio": trip_data.get("delay_ratio", 0),
        "hour_deviation": trip_data.get("hour_deviation", 12),
        "is_night_trip": trip_data.get("is_night_trip", 0),
    }])
    ...
    # Normalize
    normalized = 1 - (raw_score - score_range["min"]) / (score_range["max"] - score_range["min"] + 1e-8)
